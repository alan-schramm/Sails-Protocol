...
The dominant discovery this mission adds, beyond re-confirming known object-level lifecycle facts, is at the **cross-object** level: two real, previously-undocumented contradictions were found by direct code tracing, not assumed from absence of tests. (1) **A seller can release escrowed funds while a Dispute the buyer raised is still open** — `releaseFunds()`'s authorization check never queries the `Dispute` table at all. (2) **For two of five escrow rail types (LIGHTNING_HODL, SAFE_GUARD_EVM), a Dispute appeal can produce two independently-valid, conflicting pending fund-movement instructions** — the original ruling's unsigned transaction is never invalidated when `appeal()` draws a new arbiter, because `appeal()` never touches `Escrow`/`EscrowPendingTransaction` state at all. Both are real, reachable code paths, not theoretical.
...
| 2 | Escrow released while Dispute open | **Technically possible, undisclosed, no guard at all** | `releaseFunds()`'s `isSellerOrAssignedArbiter()` returns true unconditionally for the seller with zero `Dispute` table query anywhere in the call chain |
...
2. **Escrow release with no Dispute-open check** (§11 item 2) — a real contradiction between the Business-Rules-level expectation that a dispute pauses cooperative settlement and the Lifecycle-level fact that nothing enforces this.
...
- **Whether Escrow release should require checking for an open Dispute** (§11 item 2) — a Product/Business-Rules question this report surfaces but does not answer: is a seller's ability to release funds while a dispute is pending intentional (e.g., the dispute doesn't freeze the escrow by design) or an oversight?
...
- **Cross-object consistency** (§11): `ABSENT` as a normative domain of its own — no document anywhere previously stated any of the eight combinations' intended behavior; this report is the first place any of them is written down.
...
Two genuinely new, reachable cross-object contradictions (§11 items 2 and 3) are this mission's highest-value discovery — neither requires a crash, a race, or an edge case to reach; both are ordinary-path findings. One prior open question (F-07) is substantially de-risked by evidence of an existing, real reconciliation subsystem, though its rail-scoped boundary (MULTISIG-only) remains a genuine, disclosed gap. No frozen Offer A3 or RFC-021 B2 decision is touched. Two Product Decisions and three Architecture Decisions are surfaced for CTO adjudication; no Standard document is recommended.
