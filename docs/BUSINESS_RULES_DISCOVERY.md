...
**What is the dominant failure mode?** Not contradiction, and not a shortage of rules. The dominant failure mode is **provenance ambiguity and staleness of self-description**: (a) real, shipped, tested rules whose formal governance status trails their implementation (every RFC-021 D-item, carried over from the prior mission's F-01), (b) a comment or header describing a state of the world that either no longer holds or never fully held (`escrow.service.ts:601-609`'s destination-authority comment, describing a gap already closed before even the prior mission's own baseline — see BRD-01; `tests/sweepers.test.ts`'s header, promising coverage for two sweepers when its body genuinely, extensively tests only one — see BRD-02, corrected after CTO Gate R2), and (c) one real domain (Offer lifecycle) where the absence of a rule is not itself institutionally acknowledged from either direction — nobody has said "this is fine" or "this is a gap," it has simply never been reached. **This mission's own BRD-02 finding was itself, in its first-submitted form, an instance of failure mode (b)** — see the corrected entry below; the self-referential irony is recorded there, not smoothed over.
...
Test evidence, verified directly against the file's actual body (not its header) after CTO Gate R2: `tests/sweepers.test.ts` — 605 lines, 20 real `it()` cases across 3 `describe()` blocks, unchanged since commit `94621a2f` (2026-08-30), predating both this mission and the prior one. EXISTS/PROVES, by specific case: legacy-rail (MOCK) automatic refund on expiry, one pass over multiple rows (`"refunds every escrow with status FUNDS_LOCKED and expiresAt < now"`, line 248); non-expired/non-FUNDS_LOCKED rows correctly skipped (lines 262, 276); per-escrow failure isolation, one bad row does not block the rest (`"isolates a single failure"`, line 288); the refund is triggered with the trade's real `sellerId`, never a fabricated system actor (line 309); signature-collection rails (MULTISIG/LIGHTNING_HODL/SAFE_GUARD_EVM) route to `requiresManualRecovery`/`EXPIRED` and never call `refundFunds()` (line 347), with a real `settlement.escrow.expired` event attributed to `system:expiry-sweeper` (line 360); idempotency — a repeated sweep tick never re-transitions or re-records an already-EXPIRED escrow (lines 381, 565); exact-equality boundary timing — `evaluationTime = deadline-1` does not expire, `= deadline` expires, `= deadline+1` expires (T2/T3/T4, line 430); the disjoint MOCK-refund branch is unaffected at the identical boundary instant (T13/T18, line 474); Core-evaluation-failure and Ruleset-binding-mismatch both fail closed with no legacy fallback (T8/T21/T9, lines 510, 528); a lost race against a concurrent transition is a safe no-op, not an error (T16, line 585). **DOES NOT PROVE**: coverage of `dispute.service.ts`'s `sweepExpiredAutoResolutions()` (the QVAC auto-resolution sweeper) — this file's own header describes intending to cover that sweeper too, but its body contains zero tests for it; that sweeper's real coverage lives in `tests/disputeFlow.test.ts:862-923` instead (see BR-DISPUTE-08 and the corrected BRD-02 below).
Status: PRESENT_BUT_DISPERSED.
Decision required?: No.
...
Test evidence: `tests/disputeFlow.test.ts:862-923` (EXISTS/PROVES the advisory-only, revert-to-human contract). **Separately noted defect, corrected after CTO Gate R2**: `tests/sweepers.test.ts`'s own header comment describes intending to cover this exact sweeper (`sweepExpiredAutoResolutions()`) alongside the escrow-timelock sweeper — but its actual body (605 lines, 20 real tests, directly verified) contains genuine, extensive coverage of only the escrow-timelock sweeper; it has zero tests for `sweepExpiredAutoResolutions()` specifically. The file is not empty and its header is not a "stale, misleading" fabrication over nothing — it over-promises scope relative to what the body actually delivers. The real coverage for this specific sweeper lives in `disputeFlow.test.ts`, exactly where this rule's own primary citation already correctly points.
...
Decision required?: No new decision; the header-scope over-promise is an implementation/evidence-hygiene defect (see §10, Finding BRD-02, corrected after CTO Gate R2).
...
**BRD-02 — CORRECTED after CTO Gate R2. The original claim ("the file body is `export {}`, zero actual test bodies") was factually wrong and is retracted, not merely softened.**

Direct re-verification (full file read, line count, `describe`/`it` count, and a git-history check to rule out a stale-commit explanation) shows `tests/sweepers.test.ts` is 605 lines, contains 3 `describe()` blocks and 20 real `it()` test cases, and has been in this populated state since commit `94621a2f` (2026-08-30) — well before either this mission or the prior, frozen one. Line 34's `export {} // same forced-module reasoning as chatUnification.test.ts` is a TypeScript module-forcing idiom needed for `jest.mock` hoisting to work correctly — the same pattern the file's own header names by cross-reference — not a statement that the file contains nothing else. The original BRD-02 claim appears to have resulted from reading only as far as that line without continuing into the file's real body.

**The actual, narrower, corrected finding**: the file's header comment (lines 1-22) describes intending to cover *two* pieces of sweeper logic — `escrow.service.ts`'s `sweepExpiredEscrows()` and `dispute.service.ts`'s `sweepExpiredAutoResolutions()`. The body genuinely, extensively tests only the first (all 20 tests are scoped to `escrowService.sweepExpiredEscrows`, per BR-ESCROW-08's now-corrected citation above) — there is no test anywhere in this file for `sweepExpiredAutoResolutions()`, despite the header naming it. That second sweeper's real coverage lives entirely in `tests/disputeFlow.test.ts:862-923` instead (see BR-DISPUTE-08). This is a real, if minor, header-scope-over-promise — not an empty file, not "zero actual test bodies."

Classification: Evidence Debt, but far narrower than first claimed — a header describing two sweepers when the body covers one, not a misleading artifact over nothing. Disposition: evidence/test obligation — worth a maintainer pass to either add the missing `sweepExpiredAutoResolutions()` coverage to this file or narrow the header's own claim to match what it actually contains; not urgent, not performed here.

**This correction is itself now the primary example of this mission's own dominant failure mode (§1(b))** — a claim (BRD-02, as first submitted) whose specificity and confident phrasing made it read as verified evidence, when the underlying file was never fully read. The same discipline BRD-01's root-cause analysis named — "a comment/claim describing something is itself a claim requiring independent verification, no matter how specific it sounds" — applies here to this mission's own prior output, not only to the codebase it investigated.
...
| BRD-02 (test-file header over-promises scope; body coverage is real for what it does test) | Evidence/test obligation |
...
10. *Did I preserve uncertainty where evidence is incomplete?* **Corrected after CTO Gate R2 — this answer was stale.** `docs/ROADMAP.md` is no longer an unchecked surface: CTO Gate R1 required completing BR-OFFER-03's negative-evidence sweep, and it was searched directly (one irrelevant hit, recorded in BR-OFFER-03 above) — it is not re-listed here as outstanding. The `tests/sweepers.test.ts` uncertainty is similarly resolved, not preserved: CTO Gate R2 required a direct test-evidence sweep, which found the file fully populated and real (corrected BRD-02 above) — there is no remaining "what the header once covered" question to flag. The genuinely remaining uncertainty in this report is narrower: BR-OFFER-03's economic-consequence *Product* question (not an evidence gap — the evidence is now complete; the decision itself is what's open), and BRD-02's own narrower residual gap (whether `sweepExpiredAutoResolutions()` should gain coverage in this file or have the header's claim narrowed — an implementation/maintainer choice, not investigated further here). No uncertainty is invented merely to keep this answer populated — where the sweep is genuinely complete, it is reported as complete.
