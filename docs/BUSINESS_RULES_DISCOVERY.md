...
Current canonical home: **ABSENT on both sides** — searched `docs/PROTOCOL_SPECIFICATION.md` §1.11, `PROTOCOL_INVARIANTS.md` (no Offer-specific invariant exists at all), both ADRs (neither addresses Offer status), all 24 RFC titles/scopes, `docs/BACKLOG.md` (targeted greps for "Offer"+"lifecycle"/"transition"/"reactivat" — only two unrelated F-03/F-04 hits about a different mission's session-expiry UI numbering), and open GitHub issues (targeted searches for "Offer lifecycle", "reactivate", "OfferStatus", etc. — zero matches; `docs/ROADMAP.md` was not read and is flagged as an unchecked surface, not a confirmed-clean one).
...
Status: NOT_APPLICABLE as a "gap" — this is a clean negative finding, not a hole needing a decision. It does, however, mean F-08A's session-revocation absence (frozen by the prior mission's CTO Gate as Security/Institutional/Operational-Auth Debt) has no Business Rule amplifying its severity — a stolen, still-valid session token grants exactly the same access an ordinary valid session would, no more.
Decision required?: No — this finding closes a question, it does not open one.
Recommended destination: no action; useful negative evidence to cite if F-08A is ever revisited, so a future mission does not have to re-derive it.
...
**Not evaluated as a contradiction because it is a status update, not a disagreement:** BRD-01 below (the destination-authority comment) — the comment was accurate when written and has since become stale as the underlying code was fixed; this is staleness, not two sources currently disagreeing about the same present fact.
...
**Security Decision:** none newly identified — BR-AUTH-02's clean-absence finding actually narrows, not widens, the previously-frozen F-08A's blast radius (a stolen valid session grants ordinary access, nothing more), which is useful context for whoever eventually scopes that hardening work, not a new decision itself.
...
**BRD-01 — Destination-authority code comment is stale; the gap it describes has already been closed.** `escrow.service.ts:601-609`'s comment still describes an arbiter-override gap for LIGHTNING_HODL/SAFE_GUARD_EVM/WDK_USDT_EVM/MOCK disputes as a live, disclosed residual gap "not fixed by M8-R2." Direct code reading of `dispute.service.ts`'s actual `applyRuling()` shows `releaseToAddress`/`refundToAddress` were removed from its parameters entirely; every call site now passes `undefined`, forcing resolution through the beneficiary's own registered address for every rail. Verified via `git merge-base --is-ancestor` that commit `e4cd207` ("fix(f1-r): extend F' Destination Authority to remaining disputed rails", 2026-09-11) — which made this exact fix, and is described in its own message as closing "the residual F1 gap PR #120 explicitly left open" — is a real ancestor of this mission's current baseline. A dedicated regression test (`tests/disputeDestinationAuthorityConformance.test.ts`, 293 lines) covers all four previously-affected rails and passes; the same commit corrected 6 pre-existing assertions in `tests/disputeFlow.test.ts` that had asserted the old, vulnerable behavior. **This does not reopen or contradict anything in the already-frozen `FOUNDATIONAL_RULES_STANDARDS_INVENTORY.md`** — that report's F-06 accurately described the state of the code as it existed at that mission's own baseline; this is a genuine, dated, subsequent fix, not a correction of a prior error. The only live defect today is the stale comment itself.
Classification: implementation/documentation defect (a stale comment describing fixed behavior as still-broken). Disposition: `RESOLVE_NOW` candidate (a one-line comment edit) — not performed by this evidence-only mission.
...
No previously-tracked OPEN obligation from the prior mission was found to have silently disappeared; each was checked against this mission's own fresh evidence where relevant (RFC-021's status re-confirmed unchanged; F-08A's blast radius narrowed, not closed, by BR-AUTH-02).
...
**No — not at this time, based on what this mission actually found.** Every material rule inventoried in §3 already resolves to an identifiable canonical or near-canonical home (§5), and no contradiction found (§7) requires a new arbitrating document to resolve it. The dominant failure mode identified in §1 — provenance ambiguity and staleness of self-description — is not fixed by writing a new standard; it is fixed by (a) the RFC-021/RFC-016 governance actions already tracked, (b) two small Product Decisions, and (c) a handful of one-line comment/cross-reference corrections. Writing a new "Business Rules Standard" document now would either duplicate RFC-021/RFC-018/`DESTINATION_AUTHORITY_ARCHITECTURE.md`/`SYBIL_MITIGATION.md` (creating a second source of truth to keep in sync — the "dangerous duplication" this mission's own governing principles warn against) or become a thin index with no rules of its own — which is a legitimate future artifact, but a lightweight index is a different, much smaller thing than a "Standard," and this mission's evidence does not show it is needed urgently enough to build now rather than opportunistically later.
...
13. *Did any previously OPEN obligation disappear?* No — §10 explicitly re-affirms F-01, F-02, F-08A, F-10, and BACKLOG item 40 remain open and unchanged, and separately notes that BR-AUTH-02's finding narrows (does not close) F-08A's severity.
