...
| Lightspark Grid integration *(new — 03-implementation_plan.md §5)* | §1.5 | 🟡 **Event wiring real, API call is a stub** — `lightspark.service.ts`'s `registerLightsparkHandlers()` reacts to `intent.matched` and calls `intentEngine.transition(..., 'SETTLING', ...)` for real; `StubLightsparkClient.requestInvoice()` throws "not yet implemented," same pattern as `LightningHodlProvider` — no real Lightspark API credentials/docs available to verify against. Swapping in a real client is a one-line change inside that method |
