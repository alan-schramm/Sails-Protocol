...
| Sails OpenAgents | §1.7 (includes the `learn()` step) | 🟡 **First real capability landed** *(QVAC/WDK MVP pass, 2026-07-17)* — `qvac-risk.service.ts`, real `@qvac/sdk` local LLM inference (no cloud dependency), live-verified in this pass (see `TODO.md` §5B for timing/output notes). The "QVAC integration at 0%" finding this row previously cited (`TETHER_DUE_DILIGENCE_REPORT.md` finding 12 — that file isn't present in this environment either, same as `MASTER_COORDINATION.md`) is now out of date. Still genuinely not started: the `learn()` step, RFC-007 D7's Social Engineering Agent, and any HTTP route for this service |
