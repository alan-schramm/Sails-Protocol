...
Sails' authority model is genuinely strong in exactly one place and structurally thin everywhere else that depends on it. The **one real cryptographic authority mechanism in the entire codebase** is `resolveDispute()`'s `AuthorityDecisionPayload` — an Ed25519 signature, verified against the arbiter's registered `User.publicKey`, over a canonicalized, domain-separated economic disposition. Every other "authority" check found — Offer ownership, Trade party membership, Escrow seller/arbiter authorization, capability grants — is a durable-identity string comparison (`participantId === X`), not a cryptographic proof. This is not itself a defect: for a system whose HTTP layer already converts a signed challenge-response into a trusted `participantId` at authentication time, a downstream string comparison against that value is a legitimate, safe pattern — confirmed, not merely assumed, by this mission's systematic trace of every route's `requireAuth` derivation.

Three findings rise above "known limitation" to genuine authority gaps:

1. **The one real signature mechanism does not reach the one place its own designers intended it to matter most.** `submitTransactionSignature()` — the finalize step for MULTISIG/LIGHTNING_HODL/SAFE_GUARD_EVM — has **zero code path back to `AuthorityDecisionPayload`**. It finalizes purely on the strength of collected PSBT signatures against a pending transaction built earlier. This is the authority-angle explanation for the already-known State & Lifecycle finding that appeal leaves a prior ruling's pending instruction live: it isn't that invalidation was forgotten, it's that finalize never checked authority in the first place, for any ruling, appealed or not.
...
No frozen decision is reopened. Offer A3, RFC-021 B2, F-05/F-06/F-07, and the appeal/pending-instruction Architecture Decision are all carried forward unchanged — this mission's findings *sharpen* the appeal/pending-instruction question (it is now confirmed to be an authority-absence, not merely a state-tracking gap) without resolving it.
...
| Signed authority decisions | Yes — the one real cryptographic mechanism | `arbitration-authority.ts` |
...
| Submit PSBT signature (finalize) | any of `requiredSigners` | Signer-list membership only | pending row exists | This pending tx only | N/A | **None — no reference to `AuthorityDecisionPayload` at all** | N/A | Signature-list check only | `escrow-pending-tx.ts:293-297` |
...
**Critical test applied throughout**: is the actor authorized because of economic role, key possession, a DB role check, or merely because a caller reached the function? **Answer, uniformly**: economic role via DB-relation comparison, for every mechanism except the one signed `AuthorityDecisionPayload` path. No mechanism found relies on "merely reached the function" as its *only* protection — but `submitTransactionSignature()` comes closest to this for the specific question of "does this finalize reflect the currently-authoritative ruling," since nothing there checks authority at all (§15.5).
...
**Resolution authority**: the one real cryptographic mechanism in the system — Ed25519 signature over `AuthorityDecisionPayload`, verified against the arbiter's registered public key, plus `assertExecutionMatchesAuthorization()` confirming the signed payload matches the requested execution exactly (disputeId, escrowId, appealRound, authorityId, outcome, buyerBps).
...
- **Does the pending instruction carry authority provenance?** **No — confirmed structurally absent, not merely missing.** `submitTransactionSignature()` never references `AuthorityDecisionPayload`, the dispute row, or any authority-provenance field. It trusts the PSBT built at initiate time purely because it was built then.
...
| signer → multisig execution | Not delegation — direct exercise of one's own key | A PSBT signature IS the authorization, not a grant of it |
...
| `submitTransactionSignature()` finalize authority provenance | `UNDEFINED` — not temporal drift, structural absence of any authority reference at all |
...
- **Arbitration**: cleanly separated for the *decision* step (signed payload = 1, `resolveDispute()`'s call = 2, provider dispatch = 3, on-chain/DB result = 4) — **but this separation collapses entirely at the signature-collection finalize step**, where (1) is never referenced again and (2)-(4) proceed purely on (3)'s own historical artifact (the PSBT). This is the report's single clearest instance of "can execute" being silently treated as "is authorized" — not because a check was skipped, but because no check was ever wired to that step at all.
...
| Dispute ruling execution | **Signed `AuthorityDecisionPayload`** — the only mechanism with independently-verifiable durable evidence |
...
| Finalize-time authority (signature-collection rails) | **No durable proof referenced at all** — collected signatures prove signer participation, not ruling currency |
...
| PSBT finalize | Any required signer | Signer-list membership | `EscrowPendingTransaction` | This pending tx | No | **Undefined — no authority reference** | N/A | `escrow-pending-tx.ts:293` | Signature list only | **No authority check at all** |
...
- The appeal/pending-instruction Architecture Decision (carried forward from `docs/BACKLOG.md` item 44) is now understood, from the authority angle, to be caused by finalize having **no authority-reference mechanism at all**, not merely an unaddressed invalidation step. This is additive precision, not a correction — the original finding's substance (an Architecture Decision is required) stands exactly as frozen.
...
3. *Did I confuse economic role with key possession?* No — §22's matrix explicitly separates "role" mechanisms (DB comparison) from the one "cryptographic signature" mechanism, never conflating them.
...
8. *Did I treat execution capability as authorization?* No — §17 is built specifically to distinguish these, and names the one place (finalize) where the codebase itself does the collapsing this question warns against.
...
The authority model is stronger than a first read of "mostly string comparisons" would suggest — every comparison is fed by a genuinely trusted, authenticated identity, and the one place cryptographic proof exists (`AuthorityDecisionPayload`) is well-built. The gap is not in the mechanisms that exist, but in one mechanism's reach: it does not extend to the finalize step for signature-collection settlement, on any rail, for any ruling — which is the authority-domain root cause of the already-known appeal/pending-instruction Architecture Decision. Two new, narrower findings (the non-MULTISIG old-arbiter race, the capability-vs-finalize gap) are evidenced and queued to the same or a future Temporal/Concurrency pass. No frozen decision is touched. No new Standard is recommended.
