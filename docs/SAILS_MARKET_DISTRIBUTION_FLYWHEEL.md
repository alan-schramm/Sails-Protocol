...
| **Native SDK Participation** | A partner wallet/app embeds `@satsails/p2p-trading-sdk` directly into its own product, never touching `sails-ui` at all | **Corrected wording (P3-F07, Pre-M3 Reality Gate, 2026-09-13):** the technical path is real; the evidence for it is internal, not independent. Precisely: **native SDK technical path — real** (`SailsClient` is the same client `sails-ui` itself uses, `packages/sails-ui/src/lib/sailsClient.ts`; nothing in it is Sails-Market-specific, `docs/PRODUCT_INTERACTION_MODEL.md` §7's Wallet Integrator path); **internal/reference integration — evidenced** (`sails-ui`'s own consumption of `SailsClient` is real, working proof the SDK functions as documented); **independent partner integration — not yet evidenced** (no external team has integrated it without this team's own assistance). The original wording here ("Real, supported, evidenced") collapsed these three into one claim, which read as though third-party partner integration had occurred — it has not. Issue #99's own conclusion still governs: *"reference integration is necessary but insufficient"* — `sails-ui` proves the SDK is usable, not that anyone else has used it. |
