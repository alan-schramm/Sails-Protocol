...
describe("SailsClient — sendTransaction method", () => {
  it("signs and broadcasts a transaction using the injected wallet adapter", async () => {
    const wallet = {
      ...
      signTransaction: async (_asset: string, tx: any) => ({
        ...(tx as any),
        signed: true,
      }),
      broadcastTransaction: async (asset: string, signedTx: unknown) =>
        `0xmockhash-${asset}-${JSON.stringify(signedTx)}`,
  ...
  it("throws if no wallet is configured", async () => {
    ...
    await expect(client.sendTransaction("BTC", {})).rejects.toThrow(
      "requires a wallet adapter",
    );
  });
});
