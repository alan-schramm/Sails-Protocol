...
export function UserAvatar({ user, size = 'md' }: { user: User; size?: keyof typeof SIZES }) {
  ...
  return (
    <div className={`${SIZES[size]} shrink-0 rounded-full bg-brand-orange-accent/15 border border-brand-orange-accent/25 text-brand-orange-accent font-bold flex items-center justify-center`}>
      {initial}
    </div>
  )
}
